...
fprintf(1,'and entropic distance calculated in %d seconds.',toc);   
...
[min_diff_2, spec_ind_2] = min(cond_entr2 + 1e3*eye(ntotal),[],2); % (add 1e3 to avoid minima)
...
if(min(dissimilarities)<0.5)
    ...
    fprintf(1,'\n Some variables are very close. The distances between variables will be increased to improve visualization.');
end


%--------------------------------------------------------------------------
% Nonclassical multi-dimensional scaling (MDS):

try
Y = mdscale(dissimilarities,3,'criterion','metricstress'); 
catch %if the metricstress criterion returns an error, try a different one:
    Y = mdscale(dissimilarities,3,'criterion','strain');
end
fprintf(1,'\n MDS performed in %d seconds.\n',toc); 


%--------------------------------------------------------------------------
% Return output structure:

% Core results, needed for visualization:
Output.Y = Y;
